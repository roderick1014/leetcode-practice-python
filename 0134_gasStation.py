...
class Solution(object):
    def canCompleteCircuit(self, gas, cost):
        ...
        # If the car can aways get to the next station,
        # then surely the car will complete the circuit if it is possilbe.
        accumulate_tank = 0
        start_idx = 0

        for idx in range(len(gas)):
            accumulate_tank += (gas[idx] - cost[idx])
            if accumulate_tank < 0:
                start_idx = idx + 1
                accumulate_tank = 0

...
class Solution(object):
    def copyRandomList(self, head):
        ...
        record_node = {}
        dummy = node = Node(val = -1)

        while head:
            if head not in record_node:
                record_node[head] = Node(head.val)

            node.next = record_node[head]
            node = node.next

            if head.random:
                if head.random not in record_node:
                    record_node[head.random] = Node(val = head.random.val)
                node.random = record_node[head.random]
